...
City::City(int nRows, int nCols)
 : m_rows(nRows), m_cols(nCols), m_player(nullptr), m_nFlatulans(0), mHistory(nRows,nCols)
{
...
void City::moveFlatulans()
{
    ...
    for (int k = 0; k < m_nFlatulans; k++)
    {
        ...
        if ((rowdiff == 1 || rowdiff == -1) && (coldiff == 1 || coldiff == -1)) {
            atLeastOneDiagonal = true;
        }
    }
    // If at least one diagonal, record in history
    if (atLeastOneDiagonal)  {
        mHistory.record(m_player->row(), m_player->col());
    }
}
