...
void Game::play()
{
    ...
    while ( ! p->isPassedOut()  &&  m_city->flatulanCount() > 0)
    {
        ...
        if (action.size() == 0) {
            // player preaches
            p->preach();
            // moved to inside if statement because history does something different
            m_city->moveFlatulans();
        }
        else if (action[0] == 'h') {
            // h is a separate case to not count for player's turn
            // displays history grid
            m_city->history().display();
            cout << "Press enter to continue.";
            cin.ignore(10000,'\n');
            // redisplays actual grid
        }
        else
        {
            switch (action[0])
